...
const CatalogsAndReportsPage: React.FC<CatalogsAndReportsPageProps> = ({ assets = [] }) => {
  ...
  const availableYears = assets && assets.length > 0 
    ? [...new Set(assets.flatMap(a => a.history.map(h => h.year)))].sort((a, b) => b - a)
            ...
            <Card>
              ...
              <div className="flex flex-col md:flex-row items-center gap-4">
                ...
                <div className="flex w-full md:w-auto md:pt-6 gap-2">
                  <Button onClick={() => handleExportCSV(assets, selectedYear, locations, categories)} variant="secondary" className="w-full"><DownloadIcon className="h-4 w-4" /> Exportar a CSV</Button>
                  <Button onClick={() => handleExportPDF(assets, selectedYear, locations, categories)} variant="primary" className="w-full"><DownloadIcon className="h-4 w-4" /> Exportar a PDF</Button>
                </div>
              </div>
            </Card>
          )}
